            ...
            <div className="w-full bg-transparent">
               <div className="flex items-center w-full bg-transparent gap-[16px] h-full">
                  <div className="flex items-center gap-4 flex-1">
                     {icon && <span className={`${isActive ? "text-[#178CCB]" : "hover:text-[#178CCB]"}`}>{icon}</span>}
                     {label}
                  </div>
                  {subItems && (
                     <span
                           ...
                           <div onClick={() => toggleResubItems(index)}>
                              <NavLink
                                 className={`rounded-[4px] whitespace-nowrap text-[16px] font-normal pl-[34px] flex items-center gap-[16px] py-[10px] pe-[12px] cursor-pointer
                                    ${subItemActive ? "bg-[#E8F4FA] border-b-[#178CCB] " : ""}
                                    hover:bg-[#E8F4FA] `}

                                 to={subItem.to}
                              >
                                 <span className={subItemActive ? "text-[#178CCB]" : ""}> {subItem.icon}</span> {subItem.label}
                              </NavLink>
                           </div>
                        </div>
                     );
